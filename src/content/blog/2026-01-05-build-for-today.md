...
The stored procedure middleware was an extreme case, but the pattern underneath it was common. I spent two years at a company specialising in PHP rescue projects — we took business-critical applications abandoned by their original developers and made them maintainable again.
...
If the pattern is so costly, why do we keep falling into it?
...
At the climate tech startup, the incoming CTO felt that Docker and Kubernetes were "too complicated." So we built our own deployment infrastructure on top of LXC, a lower-level container system. Custom tooling for a custom container runtime, because someday we might need features that Docker couldn't provide.
...
The system is simple. Not simplistic — it handles the complexity it needs to handle. But it doesn't have unnecessary machinery. No plugin architecture. No elaborate design patterns wrapping straightforward logic. No configuration system that can do anything. Just code that does what it needs to do, organised clearly enough that any competent engineer can understand it.
...
None of this means you should never think ahead. Some decisions genuinely deserve careful thought — the trick is knowing which ones.

## The Irreversibility Test

Not all future-proofing is wrong.
...
For everything else, there's a principle that sounds dismissive but is one of the most valuable in software engineering.
